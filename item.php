...
if (isset($_GET["bid"])!="") {
	$sql=" select * from buy_detail where bid=$_GET[bid] ";
} else {
	...
	$sql=" select * from buy_detail where bid=$mid ";
}
...
function del_b(val1,val2) {
	if (confirm('คุณแน่ใจหรือว่าต้องการลบรายการนี้ ?')==true) {
...
<table width="680" border="1" cellspacing="0" cellpadding="0">
  <tr class="h_table">
    ...
    <td width="73">หน่วยละ</td>
    <td width="95">ราคา /บาท</td>
    <td width="51">ลบ</td>
  </tr>
  <?php
  	$sum_total=0;
  ...
  <tr>
    ...
    <td><div align="center"><?php echo number_format($rs2["price"],2); ?></div></td>
    <td><div align="center"><?php echo number_format($rs2["price"] * $rs["quantity"],2); ?></div></td>
  ...
  <tr>
    <td>&nbsp;</td>
    <td><div align="center"><strong>รวมเป็นเงินทั้งสิ้น</strong></div></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td><div align="center"><?php echo number_format($sum_total,2); ?></div></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
