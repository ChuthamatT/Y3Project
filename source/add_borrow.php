...
if (isset($_GET["identity"])!="") {
	$sql=mysql_query("select * from member where mb_identity='".$_GET["identity"]."' ");
 	if(@mysql_num_rows($sql)>=1){
		...
		$sql2=mysql_query("select * from borrow where b_identity='".$identity."' and status='1' ");
...
if(count($_SESSION['sessp_id'])=="0"){
	//ลบ session
	session_unregister("sessp_id");
	session_unregister("sess_num");
	session_register("notID");
}else{
?>
...
<?PHP
	...
	$sql = mysql_query("SELECT * FROM procurment WHERE id='".$_SESSION['sessp_id'][$i]."'");
	$rst = mysql_fetch_array($sql);
	 $ID = $rst['id'];
	$on++;
	$name = $rst['name'];
